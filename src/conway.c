...
extern int SCALE, STRICT_PASTE, WRAP;
...
uint8_t get_neighbors_wrap(const cell_t* cells, cell_t* top_row, size_t x, size_t y) {
    ...
    uint16_t xp1 = (x + 1)%W;
    uint16_t xm1 = (x + W - 1)%W;
    uint16_t yp1 = (y + 1)%H;
    uint16_t ym1 = (y + H - 1)%H;
...
uint8_t get_neighbors_no_wrap(const cell_t* cells, cell_t* top_row, size_t x, size_t y) {
    ...
    const uint16_t row = y * W;
    const uint16_t row_up = (y > 0) ? row - W : 0;
    const uint16_t row_dn = (y + 1 < H) ? row + W : row;

    const uint8_t y_0   = y > 0;
    const uint8_t x_0   = x > 0;
    const uint8_t xp1_W = x + 1 < W;
    const uint8_t yp1_H = y + 1 < H;
...
void step(cell_t* restrict buffer_main, cell_t* restrict buffer_alt) {
    ...
    */
    ...
    size_t i = 0;
    cell_t top_row[W];
    memcpy(top_row, buffer_main, W * sizeof(cell_t));

    uint8_t (*get_neighbors)(const cell_t*, cell_t*, size_t, size_t) = WRAP ? get_neighbors_wrap : get_neighbors_no_wrap;

    for (size_t y = 0; y < H; y++) {
        for (size_t x = 0; x < W; x++, i++) {
            uint8_t neighbors = get_neighbors(buffer_main, top_row, x, y);
