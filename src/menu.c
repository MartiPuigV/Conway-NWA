...
extern int COLOR_IDX, SCALE_IDX, SCALE, FRAME_MS, FONT, STRICT_PASTE, FONT, WRAP;
...
void _menu_save_config() {
    const char config[6] = { (char)SCALE_IDX, (char)COLOR_IDX, (char)FRAME_MS, (char)STRICT_PASTE, (char)FONT, (char)WRAP };
    save_config(config);
...
void _menu_toggle_wrap() {
    WRAP = !WRAP;
    display_message(WRAP ? "wrap on" : "no wrap", 7);
}
